...
// Dùng html2canvas-pro (nạp ở index.html) — hỗ trợ sẵn oklch/oklab/lab/lch/hwb/color()
// nên không cần bước tự quy đổi màu nữa.
function pageHTMLFull(body, W, H) {
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>${FONT_EMBED}*{margin:0;padding:0;box-sizing:border-box}body{width:${W}px;color:#0f172a;background:#fff;color-scheme:only light}</style></head><body>${body}</body></html>`;
}
